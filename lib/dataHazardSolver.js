class DataHazardSolver {
    ...
    getDetectionBetweenLines(firstCheck) {
        ...
        let rs1Match = srcLine[1] === this.instC[2] && srcLine[0] !== "jal";
        let rs2Match = srcLine[1] === this.instC[3] && srcLine[0].charAt(0) !== "b";
        let storeRdMatch = this.instC[0].charAt(0) === "s" && this.instC[0].length === 2 && srcLine[1] === this.instC[1];
        let memRegMatch = this.instC[4][1] !== "" && this.instC[4][1] === srcLine[1];
    ...
    insertForwardingLine(srcLineNumber, desLineNumber, startCC, endCC) {
        this.noHazardsAssemblyCode[srcLineNumber][5] = this.noHazardsAssemblyCode[srcLineNumber][5].concat([startCC, desLineNumber, endCC]);
    }
