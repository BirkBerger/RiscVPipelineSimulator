...
class PipelineTable {
    ...
    //     this.htmlTable = this.getHazardousHTMLTable(codeArray);
    // }
    //
    // /**
    ...
    //     this.htmlTable = this.getHazardousHTMLTable(hazardFreeCodeArray);
    // }
     ...
     */
    setHazardousHTMLTable(cleanCode) {

        this.hazardFreeCode = [...cleanCode];
        let numberOfColumns = (cleanCode.length+4) * 2;
        ...
        for (var j = -1; j < cleanCode.length; j = j + 1) {
            ...
            for (var k = 0; k < numberOfColumns; k++) {
                ...
                else if ((cleanCode[j][0] !== "Flush" && cleanCode[j][0] !== "Stall") && k >= j*2 && k <= j*2+8) {
                    // clock cycle columns
                    if (k%2 === 0) {
    ...
    insertFlushGraphics(rowNumber, tableFromBase) {
        ...
        for (var j = startCol; j < startCol+10; j+=2) {
            ...
            tableFromBase.rows[rowNumber].cells[j].innerHTML = '<img width="70%" src="../images/wave.png" alt="flush">';
        }
    }
