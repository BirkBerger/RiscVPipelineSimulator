...
class PipelineTable {
    ...
    //         console.log("this.hazardFreeCode[i][0]: " + this.hazardFreeCode[i][0]);
    //         if (this.hazardFreeCode[i][0].includes(":")) {
    //             console.log("attempting to delete row " + (i+1));
    //             tableFromBase.deleteRow(i+1);
    //         }
    //     }
    // }
    ...
    insertFlushGraphics(rowNumber, tableFromBase, previousFlush) {
        ...
        let endCol = startCol + (previousFlush ? 10 : 9);
        for (var j = startCol; j < endCol; j++) {
            if (j % 2 === 0) {
                tableFromBase.rows[rowNumber].cells[j].id = "flush-cell";
                tableFromBase.rows[rowNumber].cells[j].innerHTML = '<img width="103%" src="../images/wave_7.png" alt="flush">';
            } else {
