...
class AssemblyInterpreter {
    ...
    interpretCleanAssemblyCode(cleanCode) {
        ...
        while (Number(this.pc) < cleanCode.length) {
            ...
            // increment PC if a branch has not occurred
            if (this.pcChange) {
                this.pcChange = false;
            } else {
                this.pc += 1n;
    ...
    auipc() {
        ...
        console.log("rd: " + rdNumber);
        let immShifted = BigInt.asIntN(20, this.inst[3]) << 12n;
        console.log("immShifted: " + immShifted);
        console.log("this.p: " + this.p);
        console.log("result: " + BigInt.asIntN(64, this.pc + immShifted));
        this.gpRegisters[rdNumber] = BigInt.asIntN(64, this.pc + immShifted);
        this.pcChange = true;
    }
