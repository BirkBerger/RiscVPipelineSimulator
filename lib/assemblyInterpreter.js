...
class AssemblyInterpreter {
    ...
    constructor(labelsByLineNumber) {
        this.pc = 0n;
        this.pcChange = false;
    ...
    interpretCleanAssemblyCode(cleanCode) {
        ...
        while (Number(this.pc) < cleanCode.length) {
            ...
            // save current register contents in register log
            this.registerLog[this.cc] = this.gpRegisters;

            // if instruction is a label declaration
            if (this.inst.includes(":")) {
                this.pc += 1n;
                continue;
            }
        ...
        // this.setInterpretedCode(cleanCode);
        // this.setHighlightingList(cleanCode);
    }

    incrementPC() {
        if (this.pcChange) {
            this.pcChange = false;
            this.flushes.push(this.pc);
        } else {
            this.pc += 1n;
        }
    }

    // setInterpretedCode(cleanCode) {
    //     for (var i = 0; i < this.pipelineOrder.length; i++) {
    //         this.interpretedCode[i] = cleanCode[this.pipelineOrder[i]];
    //     }
    //     for (var j = 0; j < this.flushes.length; j++) {
    //         this.interpretedCode.splice(this.flushes[j],0,["Flush","","","",[false,""],[]]);
    //         this.interpretedCode.splice(this.flushes[j],0,["Flush","","","",[false,""],[]]);
    //     }
    // }

    setHighlightingList(cleanCode) {
        this.highlightingList = new Array(cleanCode.length);
        for (var i = 0; i < this.pipelineOrder.length; i++) {
    ...
    getBranchFields() {
        ...
        let branchAddress = new RegExp('^(-?[0-9]+)$').test(target) ?
                            ...
                            BigInt(this.labelsByLineNumber[target]);
        let rd = this.inst[1];
        let rs1 = this.inst[2];
        return [rd.substring(1, rd.length),
                rs1.substring(1, rs1.length),
    ...
    jal() {
        ...
        this.pc = BigInt(this.labelsByLineNumber[this.inst[2]]);
        this.pcChange = true;
    }
