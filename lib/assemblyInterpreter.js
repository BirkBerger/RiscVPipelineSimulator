...
class AssemblyInterpreter {
    ...
    interpretCleanAssemblyCode(cleanCode) {
        ...
        while (Number(this.pc) < cleanCode.length) {
            ...
            console.log("current regs: " + this.gpRegisters);
            console.log("current mem x9: " + this.dataMem[9]);
            console.log("current mem x11: " + this.dataMem[11]);
    ...
    unsignedRightShift(value, valueLength, shifts) {
        value = value >> shifts;
        let compare = BigInt.asUintN(valueLength + Number(shifts),BigInt.asIntN(valueLength - Number(shifts), 1n));
    ...
    add_sub(add) {
        ...
        console.log("updated reg " + fields[0] + ": " + this.gpRegisters[fields[0]]);
    }
    ...
    load(signed, numberOfBytes) {
        ...
        for (var i = memAddress; i < memAddress+numberOfBytes; i++) {
            ...
            memValue = memValue | BigInt.asUintN(64, this.dataMem[i]) << k;
    ...
    store(numberOfBytes) {
        ...
        for (var i = fields[1]; i < fields[1] + numberOfBytes; i++) {
            ...
            rd = this.unsignedRightShift(rd,64,8n);
        }
    }
