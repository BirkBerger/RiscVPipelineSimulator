...
class AssemblyInterpreter {
    ...
    interpretCleanAssemblyCode(cleanCode) {
        ...
        while (Number(this.pc) < cleanCode.length) {
            ...
            if (this.inst[0].includes(":")) {
                this.pipelineOrder.pop();
                this.pc += 1n;
                continue;
            }
    ...
    putFlushesInPipelineOrder() {
        // let fLushPc1 = Number(this.pc + 1n);
        // let fLushPc2 = Number(this.pc + 1n);


        // let flushPC = Number(this.pc + 1n);
        // let flushesLeft = 2;
        // for (var i = flushPC; flushPC < Math.min(this.code.length, flushesLeft+1); flushPC++) {
        //     if (this.code[i][0].includes(":")) {
        //         i++;
        //     } else {
        //         this.pipelineOrder.push([i, false]);
        //         flushesLeft--;
        //     }
        // }
        ...
        let flushPC = Number(this.pc + 1n);
        let flushesLeft = 2;
        console.log(Math.min(this.code.length, flushesLeft+1));
        while (flushPC < this.code.length && flushesLeft > 0 ) {
            console.log(flushPC);
            console.log("this.code[flushPC][0]: " + this.code[flushPC][0]);
            if (this.code[flushPC][0].includes(":")) {
                flushPC++;
            } else {
