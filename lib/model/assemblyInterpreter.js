...
class AssemblyInterpreter {
    ...
    constructor(lineNumberByLabel, initialMemory) {
        this.pc = 0n;
        this.pcChange = false;
        this.currentRegisters = new BigInt64Array(32).fill(0n);
        this.registerLog = [];
        this.currentMem = initialMemory;
        this.memLog = [];
    ...
    /**
     * Fill register log with 0-registers at the first 5 clock cycles,
     * as the first registers are logged after the first WB-stage.
     * Initialize memory log with no saved memory slots at the first 4 clock cycles,
     * as the first memory is logged after the first MEM-stage.
     */
    initializeLogs() {
        const emptyRegisters = new BigInt64Array(32).fill(0n);
        for (var i = 0; i < 4; i++) {
            this.registerLog[i] = [...emptyRegisters];
            this.updateDataMemoryLog(i);
        }
        this.registerLog[4] = [...emptyRegisters];
    }

    /**
     * Update register log:
     * Input current register values in the register log after the WB-stage
     */
    updateRegisterLog() {
        this.registerLog[this.cc + 5] = this.currentRegisters;
        this.currentRegisters = [...this.currentRegisters];
    }

    /**
     * Update memory log:
     * Input current memory values in memory log at input time (default: after the MEM-stage)
     */
    updateDataMemoryLog(cc = this.cc + 4) {
        let cleanDataMem = [];
        for (var i = 0; i < this.currentMem.length; i++) {
            let memValue = this.currentMem[i];
            if (typeof memValue !== "undefined") {
                cleanDataMem.push([i,memValue]);
            }
        }
        this.memLog[cc] = cleanDataMem;
        this.currentMem = [...this.currentMem];
    }

    /**
     * Extend logs to fill entire width of pipeline table
     */
    extendLogs() {
        // fill minimum width of 10 clock cycles
        while (this.registerLog.length < 10 || this.memLog.length < 10) {
            if (this.registerLog.length < 10) {
                this.registerLog.push(this.currentRegisters);
            }
            if (this.memLog.length < 10) {
                this.updateDataMemoryLog();
            }
            this.cc++;
        }
        // extend memory log with 1 clock cycle,
        // as the pipeline table includes 1 clock cycle after the execution of the final instruction
        this.updateDataMemoryLog();
    }
    ...
    getMemAccessFields() {
        ...
        let memAddress = Number(offset) + Number(this.currentRegisters[memReg]);
    ...
    initializeDataMemory(memAddress, finalAddress) {
        ...
        for (var i = startAddress; i <= endAddress; i++) {
            if (typeof this.currentMem[i] === "undefined") {
                this.currentMem[i] = 0n;
            }
        }
    }
    ...
    add_sub(add) {
        ...
        let rs1 = this.currentRegisters[fields[1]];
        let rs2 = this.currentRegisters[fields[2]];
        // set rd
        this.currentRegisters[fields[0]] = add ? rs1 + rs2 : rs1 - rs2;
    }
    ...
    addi() {
        ...
        // set rd
        this.currentRegisters[fields[0]] = this.currentRegisters[fields[1]] + fields[2];
    }
    ...
    addiw() {
        let fields = this.getImmediateFields();
        let rs1 = BigInt.asIntN(32, this.currentRegisters[fields[1]]);
        // set rd
        this.currentRegisters[fields[0]] = BigInt.asIntN(64, BigInt.asIntN(32,rs1 + fields[2]));
    }

    /**
     * Evaluate addw and subw instruction
     * @param add - True if addw opcode, false if subw opcode
     */
    addw_subw(addw) {
        let fields = this.getArithmeticFields();
        let rs1 = this.currentRegisters[fields[1]];
        let rs2 = this.currentRegisters[fields[2]];
        // set rd
        this.currentRegisters[fields[0]] = addw ? BigInt.asIntN(64, BigInt.asIntN(32,rs1 + rs2)) : BigInt.asIntN(64, BigInt.asIntN(32,rs1 - rs2));
    }
    ...
    and() {
        ...
        // set rd
        this.currentRegisters[rsNumbers[0]] = this.currentRegisters[rsNumbers[1]] & this.currentRegisters[rsNumbers[2]];
    }
    ...
    andi() {
        let fields = this.getImmediateFields();
        // set rd
        this.currentRegisters[fields[0]] = this.currentRegisters[fields[1]] & BigInt.asIntN(64,fields[2]);
    }
    ...
    upperImmediate(add) {
        ...
        // set rd
        this.currentRegisters[rdNumber] = add ? BigInt.asIntN(64, this.pc + immShifted) : BigInt.asIntN(64, immShifted);
    }
    ...
    branch(signed, logicalOperation) {
        ...
        let rs1 = signed ? this.currentRegisters[fields[0]] : BigInt.asUintN(64, this.currentRegisters[fields[0]]);
        let rs2 = signed ? this.currentRegisters[fields[1]] : BigInt.asUintN(64, this.currentRegisters[fields[1]]);
        let condition;
        // evaluate condition
    ...
    jal() {
        ...
        if (rdNumber !== 0) {
            ...
            this.currentRegisters[rdNumber] = this.pc + 1n;
        }
        this.putFlushesInPipelineOrder();
    ...
    jalr() {
        ...
        if (fields[0] !== 0) {
            // store next instruction address
            this.currentRegisters[fields[0]] = this.pc + 1n;
        }
        let newAddress = this.pc + BigInt(fields[1]);
        // if address is out of bounds for the input program
        if (newAddress < 0 || newAddress >= this.code.length) {
    ...
    load(signed, numberOfBytes) {
        ...
        for (var i = memAddress; i < finalAddress; i++) {
            ...
            let memByte = this.extendMemByteUnsigned(this.currentMem[i]);
            // add to memory value
            memValue = memValue | (memByte << k);
            k += 8n;
        }
        // set rd
        this.currentRegisters[fields[0]] = signed ? BigInt.asIntN(8*numberOfBytes, memValue) : memValue;
    }
    ...
    or() {
        let fields = this.getArithmeticFields();
        // set rd
        this.currentRegisters[fields[0]] = this.currentRegisters[fields[1]] | this.currentRegisters[fields[2]];
    }
    /**
    ...
    store(numberOfBytes) {
        let fields = this.getMemAccessFields();
        var rd = this.currentRegisters[fields[0]];
        ...
        for (var i = fields[1]; i < finalAddress; i++) {
            ...
            this.currentMem[i] = BigInt.asIntN(8, rd);
    ...
    shift(left, logical, numberOfBytes, shiftByImmediate) {
        ...
        let rs1 = BigInt.asIntN(64, this.currentRegisters[fields[1]]);
        let shifts = shiftByImmediate ? BigInt.asIntN(5, fields[2]) : BigInt.asIntN(5, this.currentRegisters[fields[2]]);
        ...
        // set rd
        this.currentRegisters[fields[0]] = value;
    }
    ...
    slt_sltu(signed) {
        let fields = this.getArithmeticFields();
        let rs1 = signed ? this.currentRegisters[fields[1]] : BigInt.asUintN(64, this.currentRegisters[fields[1]]);
        let rs2 = signed ? this.currentRegisters[fields[2]] : BigInt.asUintN(64, this.currentRegisters[fields[2]]);
        // set rd
        this.currentRegisters[fields[0]] = rs1 < rs2 ? 1n : 0n;
    }
    ...
    slti_sltiu(signed) {
        ...
        // set rd
        this.currentRegisters[fields[0]] = rs1 < imm ? 1n : 0n;
    }
    ...
    divide(signedRs2, numberOfBytes) {
        let numberOfBits = numberOfBytes * 8;
        let fields = this.getArithmeticFields();
        let rs1 = BigInt.asIntN(numberOfBits, this.currentRegisters[fields[1]]);
        let rs2 = signedRs2 ? BigInt.asIntN(numberOfBits, this.currentRegisters[fields[2]]) : BigInt.asUintN(numberOfBits, this.currentRegisters[fields[2]]);
        ...
        // set rd
        this.currentRegisters[fields[0]] = BigInt.asIntN(64, rs1 / rs2);
    }
    ...
    multiplyHigh(signedRs1, signedRs2) {
        let fields = this.getArithmeticFields();
        let rs1 = signedRs1 ? this.currentRegisters[fields[1]] : BigInt.asUintN(64, this.currentRegisters[fields[1]]);
        let rs2 = signedRs2 ? this.currentRegisters[fields[2]] : BigInt.asUintN(64, this.currentRegisters[fields[2]]);
        var result = BigInt.asIntN(128, BigInt(rs1 * rs2));
        result = this.unsignedRightShift(result,128,64n);
        // set rd
        this.currentRegisters[fields[0]] = BigInt.asIntN(64, result);
    }
    ...
    mulw() {
        ...
        let rs2 = this.currentRegisters[fields[2]];
        // set rd
        this.currentRegisters[fields[0]] = BigInt.asIntN(64, BigInt.asIntN(32,rs1 * rs2));
    }
    ...
    remainder(signed, regBits) {
        let fields = this.getArithmeticFields();
        let rs1 = signed ? BigInt.asIntN(regBits, this.currentRegisters[fields[1]]) : BigInt.asUintN(regBits, this.currentRegisters[fields[1]]);
        let rs2 = signed ? BigInt.asIntN(regBits, this.currentRegisters[fields[2]]) : BigInt.asUintN(regBits, this.currentRegisters[fields[2]]);
        // check for division by 0
        if (rs2 === 0n) {
            let assemblyLineNumber = this.inst[6];
            throw new DivisionByZeroException("Error in...\n\tline " + (assemblyLineNumber + 1) + ":\n\t\t- Division by zero");
        ...
        // set rd
        this.currentRegisters[fields[0]] = BigInt.asIntN(64, BigInt.asIntN(regBits,rs1 % rs2));
    }
}
