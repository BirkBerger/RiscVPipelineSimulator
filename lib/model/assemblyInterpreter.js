...
class AssemblyInterpreter {
    ...
    extendLogs() {
        ...
        while (this.registerLog.length < 10 || this.dataMemLog.length < 10) {
            if (this.registerLog.length < 10) {
                this.registerLog.push(this.gpRegisters);
            }
            if (this.dataMemLog.length < 10) {
                this.updateDataMemoryLog();
            }
            this.cc++;
        }
    ...
    /**
     * Update memory log:
     * Input current memory values in the memory log after the MEM-stage
     */
    updateDataMemoryLog() {
        let cleanDataMem = [];
    ...
    load(signed, numberOfBytes) {
        ...
        for (var i = memAddress; i < finalAddress; i++) {
            ...
            let memByte = this.extendMemByteUnsigned(this.dataMem[i]);
            // add to memory value
