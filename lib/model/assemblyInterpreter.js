...
class AssemblyInterpreter {
    ...
    constructor(lineNumberByLabel, initialMemory, withDataHazards) {
        this.pc = 0n;
        this.pcChange = false;
    ...
    interpretCleanAssemblyCode(cleanCode) {
        ...
        while (Number(this.pc) < cleanCode.length) {
            ...
            console.log("opcode: " + this.inst[0]);
            ...
            this.incrementPC();
            this.cc++;
            this.extendRegisterLog();
            this.extendMemoryLog();
            console.log(this.cc);
        }
        this.extendLogs();
    }
    ...
    extendLogs() {
        let numberOfCCs = Math.max(10, this.code.length + 6);
    ...
    extendMemoryLog() {
        ...
        this.memLog[this.cc] = [...this.memLog[this.cc-1]];

        if (typeof writtenMemoryValues !== "undefined") {
            for (var i = 0; i < writtenMemoryValues.length; i++) {
                if (typeof writtenMemoryValues[i] !== "undefined") {
                    this.memLog[this.cc][i] = writtenMemoryValues[i];
                }
            }
        }
    }
    ...
    //     console.log("initial register log:");
    ...
    //  */
    // extendRegisterLog() {
    //     console.log("set registers in log at cc " + (this.cc + 5));
    //     // console.log(this.currentRegisters);
    //     this.mergeCurrentAndExistingRegisters();
    //     this.currentRegisters = this.withDataHazards ? [...this.registerLog[this.cc + 2]] : [...this.currentRegisters];
    // }
    //
    // mergeCurrentAndExistingRegisters() {
    //     this.registerLog[this.cc + 5] = [...this.registerLog[this.cc + 4]];
    //     for (var i = 0; i < 32; i++) {
    //         if (this.currentRegisters[i] !== 0n) {
    //             this.registerLog[this.cc + 5][i] = this.currentRegisters[i];
    //         }
    //     }
    // }
    ...
    //         let readMemory = this.currentMem[i];
    //         if (typeof readMemory !== "undefined") {
    //             cleanDataMem.push([i,readMemory]);
    //         }
    //     }
    //     this.memLog[cc] = cleanDataMem;
    //     this.currentMem = !this.withDataHazards || typeof this.memLog[this.cc + 3] === "undefined" ? [...this.currentMem] : [...this.memLog[this.cc + 3]];
    //     // this.currentMem = this.withDataHazards ? this.memLog[this.cc + 3] : [...this.currentMem];
    // }
    ...
    readMemory(address) {
        ...
        while (typeof this.memLog[ccAvailable] === "undefined" || typeof this.memLog[ccFetch][address] === "undefined") {
            ccAvailable--;
        }
        return this.memLog[ccAvailable][address];
    }

    writeRegister(regNumber, value) {
        let cc = this.cc + 5;
        console.log("write at cc: " + cc);
        console.log("write value: " + value);
        if (typeof this.registerLog[cc] === "undefined") {
            this.registerLog[cc] = [];
        }
        this.registerLog[cc][regNumber] = value;
        console.log(this.registerLog[cc]);
    }

    writeMemory(value, beginAddress, endAddress) {
        let cc = this.cc + 4;
        this.initializeDataMemory(cc, beginAddress, endAddress);
        for (var i = beginAddress; i < endAddress; i++) {
            // save the right-most 8 bits into memory at memAddress
            this.memLog[cc][i] = BigInt.asIntN(8, value);
    ...
    getMemAccessFields() {
        ...
        let memAddress = Number(offset) + Number(this.readRegister(memReg, true));
     ...
     */
    initializeDataMemory(cc, memAddress, finalAddress) {
        // initialize array at cc in memory log
        if (typeof this.memLog[cc] === "undefined") {
            this.memLog[cc] = [];
        }
        ...
        for (var i = startAddress; i <= endAddress; i++) {
            if (typeof this.memLog[cc][i] === "undefined") {
                this.memLog[cc][i] = 0n;
            }
        }
    }
    ...
    add_sub(add) {
        ...
        console.log("add/sub rs1: " + rs1);
        console.log("add/sub rs2: " + rs2);
        // set rd
    ...
    addi() {
        ...
        console.log("addi rs1: " + regValue);
        console.log("addi imm: " + fields[2]);
        // set rd
