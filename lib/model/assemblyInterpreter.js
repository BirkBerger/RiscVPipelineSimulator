...
class AssemblyInterpreter {
    ...
    constructor(lineNumberByLabel, initialMemory, withDataHazards, withControlHazards) {
        ...
        this.withControlHazards = withControlHazards;
        this.interpretedCodeArray = [];
        this.highlightOrder = [];
        this.pipelineOrder = [];
    }
    ...
    interpretCleanAssemblyCode(cleanCode) {
        ...
        while (Number(this.pc) < cleanCode.length) {
            ...
            if (Date.now() > timeOut + 800) {
                throw new InfiniteLoopException("The code has an infinite loop " + this.getPipelineStateErrorMessage() + "\n");
            }
        ...
        this.extendLogs();
        this.setInterpretedCode();
    }

    /**
     * Fills interpreted code array according to the pipeline order
     */
    setInterpretedCode() {
        for (var i = 0; i < this.pipelineOrder.length; i++) {
            // insert instruction
            if (this.pipelineOrder[i][1]) {
                this.interpretedCodeArray[i] = this.code[this.pipelineOrder[i][0]].slice();
            }
            // insert flush
            else {
                this.interpretedCodeArray[i] = ["Flush", "", "", "", [false, false],[],this.highlightOrder[i]];
            }
        }
    }

    /**
     * Extends logs by the number of clock cycles occupied by pipeline,
     * or at least 10 (the smallest pipeline illustration)
     */
    extendLogs() {
    ...
    /**
     * Adds a new clock cycle to the register log.
     * Merges the new clock cycle element with register values already written for that time.
     */
    updateRegisterLog() {
        let writtenRegisterValues = this.registerLog[this.cc];
        this.registerLog[this.cc] = [...this.registerLog[this.cc-1]];

        // for all register values written already at the new cc => store it in the new cc
    ...
    /**
     * Adds a new clock cycle to the memory log.
     * Merges the new clock cycle element with memory values already written for that time.
     */
    updateMemoryLog() {
    ...
    /**
     * Set PC for new clock cycle.
     * Either decided by branch/jump instruction or just incremented by 1.
     */
    incrementPC() {
        // if PC has been decided by branch/jump instruction
        if (typeof this.pcChange[1] !== "undefined" && (!this.withControlHazards || this.pcChange[0] === this.cc)) {
            // if control hazards should be solved => input flushes
            if (!this.withControlHazards) {
                this.putFlushesInPipelineOrder();
            }
            this.pc = this.pcChange[1];
            this.pcChange = [];
        // fetch next instruction in assembly code
        } else {
            this.pc += 1n;
        }
    }
    ...
    /**
     * Reads register value from register log.
     * @param regNumber - Register number for reading
     * @param load - True if reading instruction needs the register value for memory access
     * @returns {*}
     */
    readRegister(regNumber, load = false) {
        // if code is considered having unsolved data hazards => fetch at EX or MEM, otherwise after WB
        let ccAvailable = this.cc + (this.withDataHazards ? (load ? 3 : 2) : 5);
        // if registers have not logged for the fetch cc, go back in time to fetch
    ...
    /**
     * Reads memory value from memory log.
     * @param address - Memory address reading
     * @returns {*}
     */
    readMemory(address) {
        // read memory after MEM stage
        let ccAvailable = this.cc + 4;
        // if memory have not logged for the fetch cc, go back in time to fetch
    ...
    /**
     * Writes register value in register log.
     * @param regNumber
     * @param value
     */
    writeRegister(regNumber, value) {
        // write after WB stage
        let cc = this.cc + 5;
        // if nothing is logged for this cc => create new array
    ...
    /**
     * Writes memory value in memory log.
     * @param value - Memory value to write
     * @param beginAddress - First address to write
     * @param endAddress - Last address to write
     */
    ...
    divide(signedRs2, numberOfBytes) {
        ...
        if (rs2 === 0n) {
            let assemblyLineNumber = this.inst[6];
            throw new DivisionByZeroException("Error in...\n\tline " + (assemblyLineNumber + 1) + ":\n\t\t- Division by zero " + this.getPipelineStateErrorMessage() + "\n");
        }
        // set rd
    ...
    getPipelineStateErrorMessage() {
        return  this.withDataHazards && this.withControlHazards ? "(when no hazards are solved)" :
                !this.withDataHazards && !this.withControlHazards ? "(when all hazards are solved)" :
                !this.withDataHazards ? "(when data hazards are solved)" : "(when control hazards are solved)";
    }
}
