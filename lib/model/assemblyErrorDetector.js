...
class AssemblyErrorDetector {
    ...
    branchTargetError(branchTarget, lineNumber, numberOfLines) {
        ...
        if (this.integerExp.test(branchTarget)) {
            let branchAddress = (lineNumber * 4) + parseInt(branchTarget);
            // if instruction at address is defined in assembly code
            // TODO: address logic
            // branchAddress >= 0 && branchAddress < (numberOfLines * 4)
            if (true) {
                // if (branchAddress % 4 !== 0) {
                //     return "\t\t- branch address must be divisible by 4";
                // }
                return "";
