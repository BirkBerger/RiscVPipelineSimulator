...
class AssemblyErrorDetector {
    ...
    memAddressError(memAdd) {
        ...
        // if syntax is correct
        if (this.integerExp.test(offset) && this.regNameError(reg) === "") {
            return "";
        } return "\t\t- the memory address is invalid\n";
    }
     ...
     * @param upperImmediate - True if immediate holds 20 bits, false if it holds 12
     * @returns {string} - The error string
     */
    immediateValueError(imm, upperImmediate) {
        ...
        if (this.integerExp.test(imm)) {
            ...
            // if value is out of bounds - upper immediate
            if (upperImmediate && (parseInt(imm) < -524288 || parseInt(imm) > 524287)) {
                return "\t\t- immediate must be in the interval [-524288 ; 524287]\n";
            }
            // if value is out of bounds - regular immediate
            else if (!upperImmediate && (parseInt(imm) < -2048 || parseInt(imm) > 2047)) {
                return "\t\t- immediate must be in the interval [-2048 ; 2047]\n";
            } return "";
        }
    ...
    branchTargetError(branchTarget, lineNumber, numberOfLines) {
        ...
        // if branch target is an integer
        if (this.integerExp.test(branchTarget)) {
            let assemblyBranchAddress = lineNumber + parseInt(branchTarget);
            let memBranchAddress = (4 * lineNumber) + parseInt(branchTarget) * 4;
            // if address is within the program
            if (assemblyBranchAddress >= 0 && assemblyBranchAddress < numberOfLines) {
                return "";
            } return "\t\t- branch address " + memBranchAddress + " is out of bounds\n";
        ...
        for (var i = 0; i < codeArray.length; i++) {
            ...
            if (codeArray[i].includes(":")) {
                ...
                if (!label.includes(" ") &&
                    ...
                    lineNumbersByLabels[label.substring(0,label.length-1)] = i;
                }
            }
        } return lineNumbersByLabels;
    }
}

window.AssemblyErrorDetector = AssemblyErrorDetector;
// module.exports = AssemblyErrorDetector;