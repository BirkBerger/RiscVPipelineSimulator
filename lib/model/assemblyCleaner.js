...
class AssemblyCleaner {
    ...
    cleanAssemblyCode(code, instructionSignals) {
        ...
        for (var i = 0; i < codeArray.length; i++) {
            let cleanInst = this.getCleanInstruction(codeArray[i], i, instructionSignals, i);
            if (cleanInst !== null) {
     ...
     */
    getCleanInstruction(inst, lineNumber, instructionSignals, i) {
        ...
        if (instArr.length === 1) {
            this.lineNumberByLabel[(opcode.substring(0,opcode.length-1))] = i;
            return null;
        }
