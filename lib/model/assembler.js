class Assembler {
    ...
    putAssemblyInMemory(cleanCode) {
        console.log("clean code length: " + cleanCode.length);
        // console.log(cleanCode);
        for (var i = 0; i < cleanCode.length; i++) {
            let binaryInstruction = this.instToBinary(cleanCode[i]);
            console.log("instruction:");
            console.log(cleanCode[i]);
    ...
    instToBinary(inst) {
        ...
        if (opcode.charAt(0) === "l" || opcode.includes("i")) {
            switch (opcode) {
    ...
    shortImmToBinary(immValue) {
        return BigInt.asIntN(12, immValue) << 20n;
    }
    ...
    branchTargetToBinary(target) {
        let address = new RegExp('^(-?[0-9]+)$').test(target) ?
                      0n :           // TODO
                      BigInt(this.lineNumberByLabel[target]) * 4n;
        console.log("address: " + address);
        let binaryAddress = BigInt.asIntN(12, address);
        let lowerOffset = ((BigInt.asIntN(5, binaryAddress) >> 1n) << 8n) & this.rdCompare;
        let upperOffset = (binaryAddress >> 6n) << 26n;
