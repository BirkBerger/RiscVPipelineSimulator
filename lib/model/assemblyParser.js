...
class AssemblyParser {
    ...
    //     // 1st: if instruction writes rd or memory
    //     // 2nd: if store instruction
    //     // 3rd: if load instruction
    //     for (var i = 0; i < numberOfLines; i++) {
    //         this.instructionSignals[i] = [null, false, false];
    //     }
    // }
    ...
    getInstructionErrors(inst, lineNumber, numberOfLines) {
        ...
        // 1st: Stores in rd
        // 2nd: Store/load/jalr instruction
        this.instructionSignals[lineNumber] = [true, false];
        ...
        // check syntax of fields and set instruction signals according to group
            ...
            if (opcode === "lui" || opcode === "auipc") {
                lineErrorMessage += this.ae.immediateValueError(f2, true);
                if (opcode === "auipc") {
                    this.instructionSignals[lineNumber][0] = false;
                }
            }
            ...
            // if immediate instruction
                ...
                lineErrorMessage += this.ae.regNameError(f2) + this.ae.immediateValueError(f3, false);
            }
            // if load, store, or "jalr" instruction
                ...
                this.instructionSignals[lineNumber][1] = true;
