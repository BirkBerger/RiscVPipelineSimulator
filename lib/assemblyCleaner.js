...
class AssemblyCleaner {
    ...
    cleanAssemblyCode(code, instructionSignals) {
        ...
        for (var i = 0; i < codeArray.length; i++) {
            this.cleanCode[i] = this.getCleanInstruction(codeArray[i], i, instructionSignals);
            console.log(this.cleanCode[i][0]);
            console.log(!this.cleanCode[i][0].includes(":"));
            if (!this.cleanCode[i][0].includes(":")) {
                this.highlightingList[i] = [k];
                k++;
            }
        }
        console.log("cleaner highlights:");
        console.log(this.highlightingList);
     ...
     */
    getCleanInstruction(inst, lineNumber, instructionSignals) {
    ...
    // setHighlightingList() {
    //     for (var i = 0; i < this.cleanCode.length; i++) {
    //         this.highlightingList[i] = [i];
    //     }
    // }
}
