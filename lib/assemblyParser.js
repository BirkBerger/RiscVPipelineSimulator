const AssemblyError = require('/Users/birkberger/Documents/DTU/Bachelorprojekt/bachelorThesis2/lib/assemblyError');
/**
...
class AssemblyParser {
    ...
    initializeInstructionSignals(numberOfLines) {
        ...
        // initialize the twp signals for each instruction
        // 1st: if is stores in rd or not
        // 2nd: the register name from a mem address access
        for (var i = 0; i < numberOfLines; i++) {
            this.instructionSignals[i] = [false, ""];
        }
    }
    ...
    getAllInstructionErrors() {
        ...
        this.instructionSignals = this.initializeInstructionSignals(numberOfLines);

        // detect all labels declared in code
        this.ae.collectLabelsDeclared(this.codeArray);
    ...
    getInstructionErrors(inst, lineNumber, numberOfLines) {
        ...
        // if the instruction is a label declaration
            ...
            this.instructionSignals[lineNumber][0] = true;
        }
        ...
        // check syntax of fields and set instruction signals according to group
            ...
            // if "lr.d" instruction
                ...
                this.instructionSignals[lineNumber][1] = f2.substring(f2.indexOf("("), f2.indexOf(")"));
            }
            // if "sc.d" instruction
            else if (opcode === "sc.d") {
                lineErrorMessage += this.ae.regNameError(f2) + this.ae.regMemAddressError(f3, false);
                this.instructionSignals[lineNumber][1] = f3.substring(f3.indexOf("("), f2.indexOf(")"));
            }
            // if load, store, or "jalr" instruction
                ...
                if (opcode.charAt(0) === "s") {
                    this.instructionSignals[lineNumber] = [true, f2.substring(f2.indexOf("("), f2.indexOf(")"))];
                }
            }
            // if "jal" instruction
            else if (opcode === "jal") {
