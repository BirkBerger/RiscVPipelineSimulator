...
$(document).ready(function () {
    ...
    editor.on("gutterClick", function(cm, n) {
        if (beginPipelining) {
            pipelineTable.highlightPipelineRowsOnClick(currentHighlightingList, n);
        }
    });
        ...
        if (editor.getValue() === "") {
            ...
            if (errormessage === "") {
                ...
                try {
                    ...
                    setHazardFreeSets(crudeCode, parser, cleaner);
                } catch (e) {
...
function setHazardFreeSets(crudeCode, parser, cleaner) {
    // set the three hazard free code/highlighting-sets
    setControlHazardFreeSet(crudeCode, parser, cleaner, true);
...
function setControlHazardFreeSet(crudeCode, parser, cleaner, setLogs) {
    // solve control hazards
    const hazardSolver = new HazardSolver(hazardousSet[0]);
    hazardSolver.solveControlHazard(cleaner.lineNumberByLabel);
...
function setHazardFreeSet() {
    // solve all hazards
    const hazardSolver = new HazardSolver(controlHazardFreeSet[0]);
    hazardSolver.solveDataHazards(registerLog, dataMemLog);
