    ...
    $('#pipeline-instructions-button').click(function () {
        if (!beginPipelining) {
            ...
            if (errormessage === "") {
                ...
                try {
                    // SOLVE CONTROL
                    console.log("SOLVE CONTROL");
                    let hazardSolver1 = new HazardSolver(hazardousSet[0]);
                    hazardSolver1.solveControlHazard(parser.lineNumberByLabels);
                    controlHazardFreeSet = [hazardSolver1.code, hazardSolver1.highlightingList];
                    const controlPipelineOrder = hazardSolver1.getPipelineOrder();
                    cleaner.cleanAssemblyCode(crudeCode, parser.instructionSignals);
                    ...
                    hazardSolver3.setHighlightingListDoubleSolve(controlPipelineOrder)
