...
class AssemblyCompiler {
     ...
     * @param regWriteSignalOff - The array of instruction line numbers belonging to instructions that do not store in rd (created in AssemblyParser)
     */
    storeCode(code, regWriteSignalOff, memAddressInReg) {
        var codeArray = code.split("\n");
        // iterate assembly code array
        for (var i = 0; i < codeArray.length; i++) {
            this.cleanAssemblyCode[i] = this.getTrimmedFieldsArray(codeArray[i], i, regWriteSignalOff, memAddressInReg);
        }
    }
     ...
     */
    getTrimmedFieldsArray(inst, lineNumber, instructionSignals) {
