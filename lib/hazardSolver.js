class HazardSolver {
    ...
    detectDataHazards() {
        ...
        for (var i = 0; i < this.cleanAssemblyCode.length; i++) {
            ...
            if(instruction[4] === true && rd !== "x0") {
                ...
                for (var j = i+1; j < Math.min(i+5, this.cleanAssemblyCode.length); j++) {

                    let followingInstruction = this.cleanAssemblyCode[j];
    ...
    insertForwardingLine(srcLine, desLine, startCC, endCC) {
        this.cleanAssemblyCode[srcLine] = this.cleanAssemblyCode[srcLine].concat([startCC, desLine, endCC]);
    }
